...
Woah! there are so many files here, but the one we are looking for is `brightness`.
...
937
```
Yeah these files just contain a number that's it.  the max_brightness my laptop can have is `937` and current brightness is `10`.
...
Interesting thing is that, we can put only integers in it(`0 <= brightness <= 937`)
